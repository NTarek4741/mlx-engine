...
@app.post("/api/chat")
async def chat(
    request: Annotated[ChatRequest, Body(examples=CHAT_EXAMPLES)]
):
    """
    ...
    try:
        generator, stats_collector, prompt_tokens = await engine_core(options, request.model, request)
    except Exception as e:
    ...
    try:
        if request.stream:
            return StreamingResponse(
                chat_stream(generator, request.model, stats_collector, prompt_tokens, request.logprobs, request.top_logprobs)
            )
        else:
            return await generate_output(generator, stats_collector, request, prompt_tokens)
    except Exception as e:
...
async def messages(
    params: Annotated[MessagesParams, Body(examples=MESSAGES_EXAMPLES)]
):
    """
...
async def show_model():
    ...
    Description:
        Returns detailed information about a specified model including
        its configuration, capabilities, and metadata.

    Args:
        None

    Returns:
        dict[str, str]: A dictionary containing model details and metadata.
    """
    try:
        return {"details": "Endpoint available"}
    except Exception as e:
        logger.info(f"Error showing model details: {str(e)}")
        return {"error": f"Error showing model details: {str(e)}"}
...
async def create_model(
    repo_id: str = Query(
        ...
        examples=CREATE_MODEL_REPO_ID_EXAMPLES
    ),
...
async def copy_model():
    ...
    Description:
        ...
        Currently returns a success placeholder.
    ...
    try:
        ...
        logger.info(f"Error copying model: {str(e)}")
        return {"error": f"Error copying model: {str(e)}"}
...
async def delete_model(
    model: str = Query(description="Model name to delete (e.g., 'org/model-name' or 'org/model-name:latest')")
):
    """
    Delete a model.
    ...
    try:
        ...
        if model_cache.get("load_params") and model_cache["load_params"].get("model_path") == model_path:
            model_cache.clear()
            gc.collect()

        shutil.rmtree(model_path)
        return {"status": "success", "message": f"Model '{model_name}' deleted from {model_path}"}
    except Exception as e:
...
async def clear_huggingface_cache():
    ...
    try:
        if os.path.exists(cache_dir):
            ...
            return {"status": "success", "message": f"HuggingFace cache cleared at {cache_dir}"}
        else:
            return {"status": "success", "message": "Cache directory does not exist, nothing to clear"}
    except Exception as e:
